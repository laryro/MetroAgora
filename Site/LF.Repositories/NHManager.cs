...
using LF.DBManager.Mappings;
using LF.DBManager.NHConfig;
using LF.DBManager.NHConfig.UserPassed;
...
namespace LF.DBManager
{
    public class NHManager
    {
        ...
        {
            var mapInfo = 
                new AutoMappingInfo<UserMap, User>
                {
                    BaseEntities = new[]
                    {
                        typeof (IEntity), typeof(IListableEntity)
                    }
                };
